...
| 参数 | 必填/可选 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `text` | **必填** | `str` | 无 | 要写入 Kimi 输入框并发送的文本。 |
| `model_keyword` | 可选 | `str` | `"思考"` | 模型关键字。脚本会选择第一个名称包含该关键字的模型。 |
| `user_data_dir` | 可选 | `str` | `D:\chromeCache` | Chrome 持久化缓存目录，用于复用登录态。 |
| `max_retries` | 可选 | `int` | `3` | 页面加载或发送流程失败后的最大重试次数。 |
...
| 参数 | 必填/可选 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `texts` | **必填** | `list[str]` | 无 | 要发送的文本列表。 |
| `max_concurrency` | 可选 | `int` | `3` | 最大并发标签页数。 |
| `model_keyword` | 可选 | `str` | `"思考"` | 模型关键字。每条任务都会按该关键字选择模型。 |
| `user_data_dir` | 可选 | `str` | `D:\chromeCache` | Chrome 持久化缓存目录，用于复用登录态。 |
| `max_retries` | 可选 | `int` | `3` | 页面加载或发送流程失败后的最大重试次数。 |
